    ...
    <div className="hidden md:block bg-white rounded-3xl border border-slate-100 p-6 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center bg-slate-100/50 border border-slate-200 rounded-lg p-1">
          <button onClick={() => setPeriod(7)} className={`px-4 py-1.5 text-xs font-semibold rounded-md transition-colors ${period === 7 ? 'bg-orange-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}>7j</button>
          <button onClick={() => setPeriod(30)} className={`px-4 py-1.5 text-xs font-semibold rounded-md transition-colors ${period === 30 ? 'bg-orange-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}>30j</button>
          <button onClick={() => setPeriod(90)} className={`px-4 py-1.5 text-xs font-semibold rounded-md transition-colors ${period === 90 ? 'bg-orange-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}>90j</button>
        </div>
      </div>
